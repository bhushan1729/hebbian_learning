...
As deep learning architectures—spanning dense networks, Convolutional Neural Networks (CNNs), Recurrent Neural Networks (RNNs), and Transformers—scale to unprecedented sizes, they become notoriously over-parameterized, incurring massive computational and memory costs. Biological nervous systems elegantly resolve similar efficiency challenges through the Hebbian principle where "neurons that fire together, wire together". Rather than merely pruning indiscriminately, biological neural networks actively strengthen synaptic connections participating in synchronized neural firing, and actively isolate and eventually eliminate structurally uncorrelated or idle connections. In this paper, we translationally map this principle into modern deep learning frameworks by proposing Dynamic Activity-Dependent Pruning (DADP), a novel structural sparsification algorithm that bridges backpropagation with Hebbian structural plasticity. DADP continuously evaluates and penalizes synaptic insignificance during the training process using an expected Gradient $\times$ Activation metric ($E[|a_i \cdot \frac{\partial L}{\partial y_j}|]$), permanently masking connections that consistently fail to contribute to error reduction. Extensive empirical evaluations demonstrate that DADP serves as both a structurally compressing agent and a powerful regularizer. Across varying network topologies, DADP achieves extreme sparsity ranges of 90% to over 98% with near-lossless capacity, frequently resulting in verifiable validation test accuracy improvements ranging from +0.40% to +2.20%. Ultimately, DADP offers a universally applicable, computationally efficient single-shot training methodology that accurately mimics experience-driven biological structural evolution.
