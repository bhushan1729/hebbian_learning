...
Because $y_j = w_{ij}a_i + \dots$, we know that $\frac{\partial y_j}{\partial w_{ij}} = a_i$. Therefore:
...
### 3. Why the Expectation ($\mathbb{E}$)?
...
**The Expectation ($\mathbb{E}$) acts as a Filter for Stability:**
...
I've analyzed the implementation in [model.py](../scripts/model.py) and [engine.py](../scripts/engine.py) to clarify your doubt. Here is how the pruning logic works:
