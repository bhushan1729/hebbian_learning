...
dense_baseline_acc = 85.21  # Default fallback if baseline file not found
...
for f in files:
    with open(f, 'r') as file:
        ...
        if mode == 'baseline':
            ...
            # DADP has one at thr=0.0001 (100% sparsity, 10% acc) and one at thr=6.75e-6 (100% sparsity, 10% acc)
...
# Sort points by sparsity for each method to plot continuous lines
...
# Colors and Markers matching the MNIST benchmark style
styles = {
...
# Place legend in bottom-left to avoid overlaps with the curves
plt.legend(loc='lower left', fontsize=10, frameon=True)

# Save the plot
output_path = 'results/vgg16_cifar10_experiments/vgg16_cifar10_sparsity_vs_accuracy.png'
